...
\begin{definition}
    We call $C$ \ix{connected} if there is a gallery between any two chambers. Given a subset $J\subset I$, a \ix{residue of type J} is a $J$-connected component, i.e.\ there is a gallery between any two elements of a $J$-connected component whose type is a word in $J$. The \ix{cotype} of $J$ is $I-J$. 
\end{definition}
...
\end{center}
\tr{I will create my own picture with coloured dots.}
\begin{example}
...
Given a Coxeter group $(W,S)$, take as chambers the elements of $W$, and define an $i$-adjacency by $w\sim_iws_i$, where $S=\{s_1,...,s_n\}$ are the set of generators of the Coxeter group. If the Coxeter group has Coxeter matrix $M$, we call this building a \ix{Coxeter complex of type M}.\\
...
%The vertex set of the geometric realisation of the Coxeter complex corresponds to cosets of the maximal parabolic subgroups. These maximal parabolic subgroups are formed by taking a subset of $S$ with one element removed. So these maximal parabolic subgroups are in bijection with the elements of $S$. 
...
\begin{definition}
    If a panel $p$ corresponds to the element $xW_{S\backslash \{s\}}$, we say that $p$ has \ix{type} $s$, and write $\tau(p)=s$. 
\end{definition}
...
%Now we can use theorem \eqref{isometry} to define retractions of a building. These are maps from the building to one apartment. By theorem \eqref{isometry}, we can see that an isometry from $W$ to a building $\Delta$ is uniquely determined its image and where the identity element $1\in W$ is sent. So 
...
\subsection{Retractions and shadows}
